...
@app.errorhandler(exceptions.BookingIsAlreadyUsed)  # type: ignore [arg-type]
...
def handle_booking_is_not_confirmed(exception):  # type: ignore [no-untyped-def]
    error = {"booking": ["Cette réservation n'a pas été confirmée"]}
    return jsonify(error), api_errors.ForbiddenError.status_code
...
def handle_booking_is_already_cancelled(exception):  # type: ignore [no-untyped-def]
