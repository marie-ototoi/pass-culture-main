...
@blueprint.native_v1.route("/subscription/sworn_statement", methods=["POST"])
@spectree_serialize(on_success_status=204, api=blueprint.api)
@authenticated_user_required
def create_sworn_statement_fraud_check(user: users_models.User) -> None:
    fraud_api.create_sworn_statement_fraud_check(user, "statement from /subscription/sworn_statement endpoint")
