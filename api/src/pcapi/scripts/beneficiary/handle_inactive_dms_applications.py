...
def handle_inactive_dms_applications(procedure_id: int, skip_never_eligible_applicant: bool = False) -> None:
    ...
    for draft_application in draft_applications:
        try:
            if not _has_inactivity_delay_expired(draft_application):
                continue
            if skip_never_eligible_applicant and _is_never_eligible_applicant(draft_application, procedure_id):
                continue
...
def _is_never_eligible_applicant(dms_application: dms_models.DmsApplicationResponse, procedure_id: int) -> bool:
    try:
        application_content = dms_api.parse_beneficiary_information_graphql(dms_application, procedure_id)
