...
class OnSucessfulDMSApplicationTest:
    ...
    def test_new_beneficiaries_are_recorded_with_deposit(self):
        ...
        fraud_factories.BeneficiaryFraudCheckFactory(
            user=applicant, type=fraud_models.FraudCheckType.SWORN_STATEMENT, status=fraud_models.FraudCheckStatus.OK
        )
        # when
...
class MissingSubscriptionStepsTest:
    ...
    def test_no_missing_step(self):
        ...
        fraud_factories.BeneficiaryFraudCheckFactory(
            user=user, type=fraud_models.FraudCheckType.SWORN_STATEMENT, status=fraud_models.FraudCheckStatus.OK
        )
    ...
    def test_missing_all(self):
        ...
        expected = [
            subscription_models.SubscriptionStep.PHONE_VALIDATION,
            subscription_models.SubscriptionStep.IDENTITY_CHECK,
            subscription_models.SubscriptionStep.SWORN_STATEMENT,
        ]
        assert subscription_api.get_missing_subscription_steps(user, user.eligibility) == expected
        assert not user.has_beneficiary_role
