...
describe('src | components | pages | Signup | SignupForm', () => {
  ...
  it('should redirect to accueil page if the user is logged in', async () => {
    ...
    renderSignUp(store)
    await expect(screen.findByText("I'm logged in as a pro user")).resolves.toBeInTheDocument()
  })
  ...
  it('should redirect to structure page if the user is logged in as an Admin', async () => {
    ...
    store.user.initialized = true
    renderSignUp(store)
    await expect(screen.findByText("I'm logged in as an Admin")).resolves.toBeInTheDocument()
  })
  ...
  describe('render', () => {
    ...
    describe('formValidation', () => {
      ...
      it('should enable submit button only when required inputs are filled and call pcapi with data', async () => {
        ...
        waitFor(() => expect(screen.findByText("Ma Petite structure")).resolves.toBeInTheDocument())
        ...
        await expect(screen.findByText("I'm the confirmation page")).resolves.toBeInTheDocument()
      ...
      it('should show a notification on api call error', async () => {
        ...
        waitFor(() => expect(screen.findByText("Le téléphone doit faire moins de 20 caractères")).resolves.toBeInTheDocument())
