...
export const LAST_DEPLOYED_COMMIT: string = '##LAST_DEPLOYED_COMMIT##'

export const IS_DEV: boolean = NODE_ENV === 'development'

export const CGU_URL: string = 'https://pass.culture.fr/cgu-professionnels/'
...
let apiUrlBasedOnDomain: string = ''
if (typeof window !== 'undefined') {
  apiUrlBasedOnDomain = window.location.hostname.includes('beta.gouv')
    ? process.env.API_URL_OLD || ''
    : process.env.API_URL_NEW || ''
}
export const API_URL: string = apiUrlBasedOnDomain || 'http://localhost'

let CALC_ROOT_PATH: string = ''
if (typeof window !== 'undefined') {
  CALC_ROOT_PATH = window.location.protocol + '//' + document.location.host
}

export const ROOT_PATH: string = CALC_ROOT_PATH || 'http://localhost:3001/'
